...
int main (int argc, char * argv[]) {
    ...
    const uint64_t chunk_size = sdiv(num_entries, numStreams);
    
    for (uint64_t i = 0; i < numStreams; i++) {
        const uint64_t lower = i * chunk_size;
        const uint64_t upper = min(num_entries, lower + chunk_size);
        const uint64_t width = upper - lower;
        
        cudaMemcpyAsync(data_gpu + lower, data_cpu + lower, 
               sizeof(uint64_t)*width, cudaMemcpyHostToDevice, streams[i]);
        check_last_error();
        ...
        cudaMemcpyAsync(data_cpu + lower, data_gpu + lower, 
               sizeof(uint64_t)*width, cudaMemcpyDeviceToHost, streams[i]);
    }
